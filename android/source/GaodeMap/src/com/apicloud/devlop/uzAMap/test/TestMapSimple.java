...
public class TestMapSimple implements OnMarkerClickListener {

	public TestMapSimple() {
...
//				}
				...
				if (zoom <= 3) {
					...
					bubble.setId("4");
					bubble.setLon(116.3683244);
					bubble.setLat(39.915085);
					bubble.setName("中华人民共和国");
					addBill(bubble, cameraPosition);
					removeBill();
					...
					for(int i = 0; i < 3; i++) {
						...
						if (i == 0) {
							bubble.setId(i + "");
							bubble.setName("北京");
							bubble.setLon(116.407394);
							bubble.setLat(39.904211);
						}else if (i == 1) {
							bubble.setId(i + "");
							bubble.setName("上海");
							bubble.setLon(121.473662);
							bubble.setLat(31.230372);
						}else if(i == 2){
							bubble.setId(i + "");
							bubble.setName("天津");
							bubble.setLon(117.200983);
							bubble.setLat(39.084158);
						}
						addBill(bubble, cameraPosition);
					}
				}
	...
	private void removeZhBill() {
		Marker mark = mMarkers.get("4");
		if (mark != null) {
			mark.remove();
			mMarkers.remove("4");
		}
	...
	private void removeBill() {
		for (int i = 0; i < 3; i++) {
			Marker mark = mMarkers.get(i + "");
			mark.remove();
			mMarkers.remove(i + "");
		}
	}
	private Map<String, Marker> mMarkers = new HashMap<String, Marker>();
	private void addBill(Bubble bubble, CameraPosition cameraPosition) {
		if (!mMarkers.containsKey(bubble.getId())) {
			Marker marker = mAMap.addMarker(createBillboardOptions(bubble, bubble.getLon(), bubble.getLat(), false));
			mMarkers.put(bubble.getId(), marker);
		}
	}
	
	private MarkerOptions createBillboardOptions(Bubble bubble, double lon, double lat, boolean draggable) {
		int layoutId = UZResourcesIDFinder.getResLayoutID("mo_amap_bubble_left");
		View infoContent = View.inflate(moduleContext.getContext(), layoutId, null);
		Bitmap bgImg = getBitmap("/storage/emulated/0/Movies/paopao.png");
		...
		//BitmapDescriptor bitmapDescriptor = BitmapDescriptorFactory.fromPath(bgImgStr);
