...
public class BusRouteOverlay extends RouteOverlay {
	...
	private void addBusLineSteps(List<LatLonPoint> listPoints) {
		...
		String img = getBusStrokeImg();
		if (TextUtils.isEmpty(img)) {
			addPolyLine(new PolylineOptions().width(getRouteWidth())
					.color(getBusColor())
					.addAll(AMapUtil.convertArrList(listPoints)).setDottedLine(getBusLineDash()));
		}else {
			Bitmap bitmap = UZUtility.getLocalImage(img);
			if (bitmap != null) {
				addPolyLine(new PolylineOptions().width(getRouteWidth())
						.color(getBusColor())
						.addAll(AMapUtil.convertArrList(listPoints)).setDottedLine(getBusLineDash()).setCustomTexture(BitmapDescriptorFactory.fromBitmap(bitmap)));
			}
		}
...
//				.addAll(AMapUtil.convertArrList(listPoints)));
	...
	private void addWalkPolyline(LatLng latLngFrom, LatLng latLngTo) {
		String img = getWalkStrokeImg();
		if (TextUtils.isEmpty(img)) {
			addPolyLine(new PolylineOptions().add(latLngFrom, latLngTo)
					.width(getRouteWidth()).color(getWalkColor()).setDottedLine(getWalkLineDash()));
		}else {
			Bitmap bitmap = UZUtility.getLocalImage(img);
			if (bitmap != null) {
				addPolyLine(new PolylineOptions().add(latLngFrom, latLngTo)
						.width(getRouteWidth()).color(getWalkColor()).setDottedLine(getWalkLineDash()).setCustomTexture(BitmapDescriptorFactory.fromBitmap(bitmap)));
			}
		}
	...
	private void addWalkPolyline(List<LatLng> listWalkPolyline) {
		...
		if (TextUtils.isEmpty(img)) {
			addPolyLine(new PolylineOptions().addAll(listWalkPolyline)
					.color(getWalkColor()).width(getRouteWidth()).setDottedLine(getWalkLineDash()));
		}else {
			Bitmap bitmap = UZUtility.getLocalImage(img);
			if (bitmap != null) {
				addPolyLine(new PolylineOptions().addAll(listWalkPolyline)
						.color(getWalkColor()).width(getRouteWidth()).setDottedLine(getWalkLineDash()).setCustomTexture(BitmapDescriptorFactory.fromBitmap(bitmap)));
			}
		}
	...
	public void drawLineArrow(LatLng latLngFrom, LatLng latLngTo) {
		...
		if (TextUtils.isEmpty(img)) {
			addPolyLine(new PolylineOptions().add(latLngFrom, latLngTo).width(3)
					.color(getBusColor()).width(getRouteWidth()).setDottedLine(getBusLineDash()));// 绘制直线
		}else {
			Bitmap bitmap = UZUtility.getLocalImage(img);
			if (bitmap != null) {
				addPolyLine(new PolylineOptions().add(latLngFrom, latLngTo).width(3)
						.color(getBusColor()).width(getRouteWidth()).setDottedLine(getBusLineDash()).setCustomTexture(BitmapDescriptorFactory.fromBitmap(bitmap)));// 绘制直线
			}
		}
