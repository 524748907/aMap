...
//			}
	...
	private void initPolylineOptions() {
		...
		String img = getStrokeImg();
		if (TextUtils.isEmpty(img)) {
			mPolylineOptions.color(getDriveColor()).width(getRouteWidth()).setDottedLine(getDriveLineDash());
		}else {
			Bitmap bitmap = UZUtility.getLocalImage(img);
			if (bitmap != null) {
				mPolylineOptions.color(getDriveColor()).width(getRouteWidth()).setDottedLine(getDriveLineDash()).setCustomTexture(BitmapDescriptorFactory.fromBitmap(bitmap));
			}
		}
	...
	private void colorWayUpdate(List<TMC> tmcSection) {
		...
		mPolylineOptionscolor.setDottedLine(getDriveLineDash());
		String img = getDriveStrokeImg();
		if (!TextUtils.isEmpty(img)) {
			Bitmap bitmap = UZUtility.getLocalImage(img);
			if (bitmap != null) {
	...
	private void addDrivingStationMarkers(DriveStep driveStep, LatLng latLng) {
		addStationMarker(new MarkerOptions().position(latLng)
				.title("\u65B9\u5411:" + driveStep.getAction() + "\n\u9053\u8DEF:" + driveStep.getRoad())
				.snippet(driveStep.getInstruction()).visible(nodeIconVisible).anchor(0.5f, 0.5f)
	...
	protected LatLngBounds getLatLngBounds() {
		...
		if (this.throughPointList != null && this.throughPointList.size() > 0) {
			for (int i = 0; i < this.throughPointList.size(); i++) {
				b.include(new LatLng(this.throughPointList.get(i).getLatitude(),
			...
			for (int i = 0; i < this.throughPointList.size(); i++) {
				...
				if (latLonPoint != null) {
					throughPointMarkerList.add(mAMap.addMarker((new MarkerOptions())
							...
							.visible(throughPointMarkerVisible).icon(getThroughPointBitDes())
							.title("\u9014\u7ECF\u70B9")));
				}
			}
		}
	...
	private BitmapDescriptor getThroughPointBitDes() {
		return BitmapDescriptorFactory.fromResource(UZResourcesIDFinder.getResDrawableID("amap_through"));
