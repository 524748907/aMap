...
public class RideRouteOverlay extends RouteOverlay {
	...
	public void addToMap() {
		...
		try {
			...
			for (int i = 0; i < ridePaths.size(); i++) {
				...
				LatLng latLng = AMapUtil.convertToLatLng(rideStep.getPolyline().get(0));
	...
	private void addRideStationMarkers(RideStep rideStep, LatLng position) {
		addStationMarker(new MarkerOptions().position(position)
				.title("\u65B9\u5411:" + rideStep.getAction() + "\n\u9053\u8DEF:" + rideStep.getRoad())
				.snippet(rideStep.getInstruction()).visible(nodeIconVisible).anchor(0.5f, 0.5f)
				.icon(rideStationDescriptor));
	}
	...
	private void initPolylineOptions() {
		...
		if (rideStationDescriptor == null) {
			if (TextUtils.isEmpty(getRidePointImg())) {
				rideStationDescriptor = BitmapDescriptorFactory
						.fromResource(UZResourcesIDFinder.getResDrawableID("amap_ride"));
			}else {
				rideStationDescriptor = BitmapDescriptorFactory.fromBitmap(UZUtility.getLocalImage(getRidePointImg()));
			}
			
		}
		mPolylineOptions = null;
		mPolylineOptions = new PolylineOptions();
		String strokeImg = getStrokeImg();
		if (TextUtils.isEmpty(strokeImg)) {
			mPolylineOptions.color(getRideColor()).width(getRouteWidth()).setDottedLine(getLineDash());
		}else {
			Bitmap bitmap = UZUtility.getLocalImage(strokeImg);
			if (bitmap != null) {
				mPolylineOptions.color(getRideColor()).width(getRouteWidth()).setDottedLine(getLineDash()).setCustomTexture(BitmapDescriptorFactory.fromBitmap(bitmap));
			}
		}
